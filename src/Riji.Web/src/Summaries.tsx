...
export function Summaries({ state }: { state: Snapshot }) {
  ...
  return <>
    ...
    {current && <section className="panel history summary-result"><div className="section-head"><h2>✧ {new Date(current.range.start).toLocaleDateString('zh-CN')} · {labels[current.state]}</h2><span>{current.sourceCount} 条来源 · 已完成 {current.completedBatches} 批</span></div>
