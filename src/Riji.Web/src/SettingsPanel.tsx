...
function Section({ title, meta, children }: { title: string; meta?: React.ReactNode; children: React.ReactNode }) {
  return <section className="settings-section"><div className="settings-section-heading"><h2>{title}</h2>{meta}</div>{children}</section>;
}
...
function PromptModal({ open, title, description, value, defaultValue, busy, onClose, onSave }: {
  open: boolean;
  title: string;
  description: string;
  value: string;
  ...
  if (!open) return null;
  return <div className="overlay prompt-overlay" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <section className="modal prompt-modal" role="dialog" aria-modal="true" aria-labelledby="prompt-modal-title">
      <div className="section-head"><div><span className="eyebrow">PROMPT</span><h2 id="prompt-modal-title">{title}</h2></div><button type="button" disabled={busy} onClick={onClose} aria-label="关闭">×</button></div>
      <p>{description}</p>
      <textarea autoFocus rows={10} maxLength={10000} value={draft} disabled={busy} onChange={event => setDraft(event.target.value)} />
      <div className="modal-footer"><button type="button" disabled={busy} onClick={onClose}>取消</button><button type="button" disabled={busy} onClick={() => setDraft(defaultValue)}>恢复默认提示词</button><button type="button" className="primary" disabled={busy || !draft.trim()} onClick={() => void onSave(draft)}>保存提示词</button></div>
    </section>
  </div>;
}
...
function CaptureSettingsPanel({ state, busy, run }: { state: Snapshot; busy: boolean; run: Props['run'] }) {
  ...
  const savePrompt = async (value: string) => { const ok = await saveCapture({ prompt: value }); if (ok) { setPrompt(value); setPromptOpen(false); } return ok; };
  const pending = current.jobs.filter(job => job.status !== 'Succeeded').reduce((total, job) => total + job.count, 0);
  return <>
    {(pending > 0 || current.error) && <div className="settings-status settings-status-warning"><span aria-hidden="true">!</span><div><strong>{current.error ? '识别遇到问题' : `${pending} 个任务待处理`}</strong><small>{current.error ?? '未完成任务会按当前重试策略继续处理。'}</small></div></div>}
    <Section title="采集">
    ...
    <Section title="截图识别提示词"><SettingRow label="截图识别提示词" help="设置描述重点、表达方式和分类原则；已有任务重试沿用采样时的提示词、分类与辅助信息。"><button className="settings-link" disabled={busy} onClick={() => setPromptOpen(true)}>编辑提示词 ›</button></SettingRow></Section>
    <Section title="识别状态">
    ...
    <PromptModal open={promptOpen} title="截图识别提示词" description="新采样使用修改后的提示词；已有任务重试沿用采样时的提示词、分类和辅助信息。" value={prompt} defaultValue={current.defaultPrompt} busy={busy} onClose={() => setPromptOpen(false)} onSave={savePrompt} />
  </>;
}
...
function ReviewSettings({ state, busy, run }: { state: Snapshot; busy: boolean; run: Props['run'] }) {
  ...
  const [promptOpen, setPromptOpen] = useState(false);
  const categorySource = JSON.stringify(state.recognition.categories);
  useEffect(() => setCategories(state.recognition.categories), [categorySource]);
...
function DataSettings({ state, busy, run }: { state: Snapshot; busy: boolean; run: Props['run'] }) {
  return <>
    <Section title="应用">
      <SettingRow label="主题" help="选择深色、浅色，或跟随系统。"><select value={state.settings.followSystemTheme ? 'system' : state.settings.theme} onChange={event => void run('settings', { settings: { ...state.settings, theme: event.target.value === 'system' ? state.settings.theme : event.target.value as Settings['theme'], followSystemTheme: event.target.value === 'system' } })}><option value="system">跟随系统</option><option value="dark">深色</option><option value="light">浅色</option></select></SettingRow>
      <SettingRow label="开机自启" help="登录 Windows 后自动启动。"><Switch label="开机自启" checked={state.settings.startWithWindows} disabled={busy} onChange={value => void run('settings', { settings: { ...state.settings, startWithWindows: value } })} /></SettingRow>
    ...
    <Section title="数据与备份"><SettingRow label="导出备份" help="包含记录、分类、预设、总结、对话和仍保留的关联截图，不包含 API Key。"><button disabled={busy || state.maintenance} onClick={() => void run('exportData')}>导出备份</button></SettingRow><SettingRow label="导入并替换" help="原数据先自动备份；导入后自动记录与截图保持关闭。"><button disabled={busy || state.maintenance} onClick={() => void run('importData')}>导入并替换</button></SettingRow><SettingRow label="数据位置" help="当前数据目录。点击打开将在文件资源管理器中定位该文件夹。"><span className="data-path">{state.dataPath}</span><button disabled={busy} onClick={() => void run('openDataFolder')}>打开</button></SettingRow><SettingRow label="清空记录" help="清空前会要求再次确认。"><button className="danger" disabled={busy || state.maintenance} onClick={() => void run('clearData')}>清空记录</button></SettingRow></Section>
    <Section title="关于"><SettingRow label="当前版本" help="以正在运行的桌面程序版本为准。"><span>日迹 {state.appVersion}</span></SettingRow></Section>
  </>;
}
